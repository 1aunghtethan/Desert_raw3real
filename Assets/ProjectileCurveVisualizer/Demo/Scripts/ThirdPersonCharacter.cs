    ...
    void Update()
    {
        if (Settings.mouseControl)
        {
            ...
            else if (Input.GetKeyUp(KeyCode.Space))
            {
                ...
                if (Mathf.Abs(characterRigidbody.linearVelocity.y) < 0.05f)
    ...
    void ProjectileModeLogic() {
        if (inProjectileMode)
        {
            ...
            if (Input.GetMouseButtonUp(0))
            {
                ...
                ProjectileCurveVisualizerSystem.Projectile projectile = GameObject.Instantiate(projectileGameObject).GetComponent<ProjectileCurveVisualizerSystem.Projectile>();
